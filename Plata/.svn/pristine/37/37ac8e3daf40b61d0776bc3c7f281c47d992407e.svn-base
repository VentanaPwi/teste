...
lcBUILDER = INPUTBOX('TIPO DE BUILDER: 1=DIGITAÇÃO, 3=SUPERGRADE','PWI FORM-BUIDER','1')
IF VAL(lcBUILDER)<=0
	RETURN
ENDIF
IF VAL(lcBUILDER)=1
...
<<TEXTO>> THISFORM.GRADE1.MYCONTEXTO NOSHOW PRETEXT 7
	|\<Alterar			|DO FORM [<<FORMDIG>>_DIGITAR] WITH TMPGRADE.PK_ID TO luREFRESH					|
	|\<Incluir			|DO FORM [<<FORMDIG>>_DIGITAR] WITH <<PK0>> TO luREFRESH						|
	|\<Zoom				|DO FORM [<<FORMDIG>>_DIGITAR] WITH TMPGRADE.PK_ID, 'SOLEITURA' TO luREFRESH	|
