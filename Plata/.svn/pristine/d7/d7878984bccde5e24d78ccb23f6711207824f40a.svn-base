...
* CTRL+F1 SERÁ ATALHO PARA O WTBOX
...
*******************
	...
	DO CASE
		...
		OTHERWISE
			WAIT WINDOW ('OBJETO NÃO DEFINIDO: ' + CHR(13) + ALLTRIM(loOBJETO.BASECLASS) + ': ' + ALLTRIM(loOBJETO.NAME)) AT MROW(_SCREEN.Name,0), MCOL(_SCREEN.Name,0)
			RETURN(.F.)
	ENDCASE
...
DEFINE CLASS myTOOLBAR AS Toolbar
	...
	ADD OBJECT cmdSTOBGER AS COMMANDBUTTON WITH;
		caption = ' ',;
		tooltiptext = 'Gerenciador de versões-stob',;
		SPECIALEFFECT = 2,;
		VISIBLE=.T.,;
		WIDTH = 24,;
		HEIGHT = 24,;
...
*			KEYBOARD '{F8}'
	...
	********************
		...
		ELSE
			* SE AINDA ASSIM NÃO EXISTIR ARQUIVO DE HOTKEYS, USA MÉTODO ANTIGO DA HOTKEY
			...
			TEXT TO lcMENSAGEM NOSHOW PRETEXT 2
				...
				----
				OBS: não foi encontrado o arquivo TXT com dicas completas de hetkeys
			ENDTEXT
...
*!*				MESSAGEBOX('Objeto de conexão não encontrado.',48,'Aviso')
*!*				RETURN
*!*			ENDIF
*!*			IF goCONEXAO.ALCA<=0
*!*				MESSAGEBOX('Não existe nenhuma conexão.',48,'Aviso')
*!*				RETURN
*!*			ENDIF
*!*		
*!*			DO FORM CONSTRAINT_GERENCIAR.SCX
	...
	************************
		...
		IF NOT FILE(lcTOOLS)
			MESSAGEBOX('Starter do Utilitário F8 não encontrado:' + CHR(13) + lcTOOLS, 48, 'Aviso')
			RETURN .F.
		ENDIF
		DO (lcTOOLS)
	ENDPROC
	...
	************************
		...
		* TENTA EFETUAR A RECONEXÃO
		...
		IF lnALCA > 0
			...
			MESSAGEBOX('Reconexão efetuada com sucesso!')
		ELSE
		...
		* DEFINE MENU DE CONTEXTO A SER EXIBIDO.
		DEFINE POPUP SVN_MENUPRINCIPAL FROM MROW(), MCOL() MARGIN SHORTCUT
			* DEFINE OPÇÕES DO MENU
			...
			DEFINE BAR 2 OF SVN_MENUPRINCIPAL PROMPT "Atualizar arquivos de configuração do Tortoise SVN"
